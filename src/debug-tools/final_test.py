...
def solve(X13, nloop=100, x3max=0.4):
...
for idx, X13 in enumerate([0.62, 0.65]):
    res, rate = solve(X13, nloop=100, x3max=0.4)
    ax = axes[idx]
    if len(res) > 3:
    ...
    ax.set_title(f'Tol X13={X13} rate={rate:.1%} n=100')
    ax.set_xlim(0,1); ax.set_ylim(0,0.45)
    ax.set_xlabel('x1 (L8-Bo)'); ax.set_ylabel('x3 (PM6)')
    ax.legend(); ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('final_test_Tol.png', dpi=200)
print('Saved final_test_Tol.png')
plt.show()
