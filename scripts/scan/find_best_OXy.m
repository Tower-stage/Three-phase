...
%% 目标: v3=100000 附近, 找最佳的 X13/g23 组合
v3_values = [80000, 100000, 120000];
X13_values = [0.80, 0.85, 0.90, 0.95, 1.00];
g23_values = [0.385, 0.412, 0.50, 0.60];
...
guesses = {[0.10, 0.80, 0.15]; [0.15, 0.70, 0.20]; [0.20, 0.60, 0.25]; ...
           [0.25, 0.50, 0.30]; [0.12, 0.75, 0.18]; [0.08, 0.85, 0.12]};

results_ox = cell(nv, nx, ng);
succ_ox = NaN(nv, nx, ng);
score_ox = NaN(nv, nx, ng);
cp_ox = NaN(nv, nx, ng);

fprintf('O-Xy 参数扫描: %d x %d x %d = %d\n', nv, nx, ng, nv*nx*ng);
cnt = 0;
...
for ig = 1:ng
    ...
    dg_ = 4*p(1)*u2.^3+3*p(2)*u2.^2+2*p(3)*u2+p(4);
    d2g = 12*p(1)*u2.^2+6*p(2)*u2+2*p(3);
    G22 = 1./X1+s./X2-2*g12+2*(1-2*u2).*dg_+u1.*u2.*d2g;
    G23 = 1./X1-(g12+X13)+s*g23+u2.*(1-3*u2).*dg_+u1.*u2.^2.*d2g;
    G33 = 1./X1+r./X3-2*X13-2*u2.^3.*dg_+u2.^3.*u1.*d2g;
    ...
    elseif isnan(x3_crit), x3_crit=0.01;x1_crit=0.2; end

    % Binodal (50 points for speed)
    ...
    for ii = 1:n_loop
        ...
        for igg = 1:length(guesses)
            ...
            try
                [xt,fv] = fsolve(@(x) chempot_ox(x, x3d, v1, v2, v3, s, r, X13, g23, p), x0, opts);
        ...
        if isempty(best)&&~isempty(prev)
            try
                [xt,fv]=fsolve(@(x) chempot_ox(x, x3d, v1, v2, v3, s, r, X13, g23, p), prev, opts);
                fv2=sum(fv.^2); x3c=1-xt(1)-xt(2);
    ...
    x_sol(isnan(x_sol(:,1)),:)=[]; x_sol(x_sol(:,1)>1e-6,:)=[];
    ...
    succ_ox(iv,ix,ig) = np_final/n_loop*100;
    cp_ox(iv,ix,ig) = x3_crit;

    if np_final >= 5
        c3=x_sol(1:2:end,3); c2=x_sol(1:2:end,4);
        d3=x_sol(2:2:end,3); d2=x_sol(2:2:end,4);
        c1=x_sol(1:2:end,2); d1=x_sol(2:2:end,2);
        x3cr = c3(1)-c3(end);
        distL8 = min(c1); distPM6 = min(d3(d3>1e-4));
        balance = 1-abs(distL8-distPM6);
        score_ox(iv,ix,ig) = x3cr*max(0,balance)*(succ_ox(iv,ix,ig)/100);
    end

    results_ox{iv,ix,ig} = struct('v3',v3,'X13',X13,'g23',g23,'x_sol',x_sol,...
        'x1_crit',x1_crit,'x3_crit',x3_crit,'spin_segs',{spin_segs});

    if mod(cnt, 15)==0
        fprintf(' [%d/%d] v3=%.0f X13=%.2f g23=%.3f | succ=%d\n',cnt,nv*nx*ng,v3,X13,g23,np_final);
    end
end
end
end
...
%% ===== 排序 =====
fprintf('\n===== O-Xy TOP 15 =====\n');
all_s = score_ox(:); all_s(isnan(all_s)) = -Inf;
[~, si] = sort(all_s, 'descend');
for k = 1:min(15, sum(all_s>0))
    [iv,ix,ig] = ind2sub([nv,nx,ng], si(k));
    fprintf('%2d  v3=%-6.0f X13=%.2f g23=%.3f succ=%4.0f%% score=%.4f CP=%.4f\n', ...
        k, v3_values(iv), X13_values(ix), g23_values(ig), ...
        succ_ox(iv,ix,ig), score_ox(iv,ix,ig), cp_ox(iv,ix,ig));
end
...
function F = chempot_ox(x, x3d, v1, v2, v3, s, r, X13, g23, p)
