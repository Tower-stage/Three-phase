...
%% 扫描范围基于PM6最佳参数(v3=100000, X13=0.95, g23=0.385)向外扩展
v3_values  = [60000, 80000, 100000, 150000, 200000, 300000];
X13_values = [0.55, 0.65, 0.75, 0.85, 0.95, 1.05];
g23_values = [0.30, 0.385, 0.45, 0.55];

nv = length(v3_values); nx = length(X13_values); ng = length(g23_values);
fprintf('D18-Tol 扫描: %d 组合\n', nv*nx*ng);
...
for ig = 1:ng
    ...
    for ii=1:n_loop
        ...
        for igg=1:length(guesses)
            ...
            try
                [xt,fv]=fsolve(@(x) chempot_D18(x,x3d,v1,v2,v3,s,r,X13,g23,p),x0,opts);
        ...
        if isempty(best)&&~isempty(prev)
            try
                [xt,fv]=fsolve(@(x) chempot_D18(x,x3d,v1,v2,v3,s,r,X13,g23,p),prev,opts);
                fv2=sum(fv.^2); x3c=1-xt(1)-xt(2);
...
% 保存
save(fullfile(tempDir, 'scan_D18_Tol.mat'), 'v3_values','X13_values','g23_values','succ_rate','shape_score','all_res');

function F=chempot_D18(x,x3d,v1,v2,v3,s,r,X13,g23,p)
